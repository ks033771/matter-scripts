/* ==========================================================
   ...
   - SVG inline im JS, Farben aus CSS-Variablen
   ========================================================== */
...
/* ---------- Shapes vorbereiten: Vertices + Centroid nur EINMAL ---------- */
...
function buildInstanceFromDef(def) {
  prepareShape(def); // stellt sicher, dass _verts/_centroid/_path2d gesetzt sind

  const { fill, stroke, strokeWidth } = resolvePaintFromVars(def);
  const scaleFactor = currentScale();

  // WICHTIG: _verts NICHT in-place skalieren → clone
...
/* ---------- Instanzen erzeugen (mit vorbereiteten Shapes) ---------- */
const instances = [];
for (const selector of Object.keys(SHAPE_COUNTS)) {
  ...
  prepareShape(def); // teure Arbeit nur 1x pro Shape

  const count = SHAPE_COUNTS[selector] || 1;
  for (let i = 0; i < count; i++) {
    instances.push(buildInstanceFromDef(def));
  }
}
